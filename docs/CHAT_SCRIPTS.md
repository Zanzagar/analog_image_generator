...
```
Show me how to set Task Master main model to gpt-5-codex for later: the exact command and where it writes config.
Do not run it now.
```
...
- Create src/analog_image_generator/ with __init__.py (expose __version__) and modules: geologic_generators.py, interactive.py, stats.py, reporting.py, utils.py
- Update pyproject.toml to PEP 621 + Hatchling (build-system hatchling; [project] metadata)
- Update CI to build and install the package before tests (python -m build; pip install -e .[dev])
- Ensure tests and notebooks import analog_image_generator (no relative imports)
```
...
- .taskmaster/docs/prd.txt → fluvial-v1
- .taskmaster/docs/prd_aeolian.txt → aeolian-v1
- .taskmaster/docs/prd_estuarine.txt → estuarine-v1
